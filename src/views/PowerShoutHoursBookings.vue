<template>
  <div class="h-full bg-white">
    <div
      class="flex-grow-0 text-left w-full top bottom text-white "
      style="background: #EC5C32;"
    >
      <calandarNotification
        v-if="showNotification"
        class="absolute top-3 z-50 w-full"
        @click="dismissNotificationCalander()"
      />
      <TopNavigation
        title="Power Shout"
        conversion-active="false"
        link="/powershouthours"
      />  
      <div class="h-4" />
    </div>
    <img
              ...
              <div class="text-xs text-left flex line-1 w-full pl-1">
                ...
                >
                  ...
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6 8.46289L9 11L6 13.7974V8.46289Z"
                    fill="#464958"
                  />
                </svg>
    ...
    <div class="bg-white p-2 block">
      ...
      <div class="mt-6 overflow-hidden  grid grid-cols-7 px-4 py-2">
        <div class="flex items-center justify-center text-xs p-2 h-auto uppercase">
          <div>Mon</div>
        </div>
        <div class="flex items-center justify-center text-xs p-2 h-auto uppercase">
          <div>Tue</div>
        ...
        <div
          ...
          :key="index"
          class="flex items-center justify-center text-xs p-2 cursor-pointer rounded-2xl"
          v-html="day"
        />
        <div
          ...
          :key="days"
          class="flex items-center justify-center text-xs p-2 cursor-pointer rounded-2xl"
          :class="[startDate == days ? 'bg-background text-white' : '', currentDateStd == days ? 'text-m font-bold text-bg' : '']"
          @click="selectDates(days)"
          v-html="days.slice(-2)"
        />
      </div>
      <div
        v-if="startDate.length > 0"
        class="mt-2 p-5 w-full flex justify-center items-center bg-white m-auto border"
      >
        <div class="flex">
          <select
            v-model="startHours"
    ...
    <div class="bg-white pb-4 px-4">
      <button
        type="button"
        class="cursor-pointer w-full text-center bg-background p-2 mt-2 rounded-3xl"
        @click="addBooking()"
      >
